...
class LancamentoServiceTest {
    ...
    void deveAtualizarUmLancamento() throws Exception {
        ...
        given(repository.findLancamentoByIdAndUsuarioId(lancamentoId, usuario.getId())).willReturn(Optional.of(existente));
    ...
    void deveDeletarLancamentoQuandoPendente() throws Exception {
        ...
        given(usuarioService.obterIdUsuarioPorEmail(email)).willReturn(usuario);
        given(repository.findLancamentoByIdAndUsuarioId(lancamentoId, usuario.getId())).willReturn(Optional.of(lancamentoPendente));
    ...
    void deveBuscarComTodosFiltrosEComCategorias() {
        ...
        given(repository.findAll(any(org.springframework.data.jpa.domain.Specification.class))).willReturn(listaEsperada);
    ...
    void obterPorIdLancamentoDeveRetornarQuandoEncontrado() throws Exception {
        ...
        given(usuarioService.obterIdUsuarioPorEmail(email)).willReturn(usuario);
        given(repository.findLancamentoByIdAndUsuarioId(lancamentoId, usuario.getId())).willReturn(Optional.of(lancamentoEsperado));
    ...
    void obterSaldoPorUsuarioDeveSubtrairReceitasEDespesas() throws Exception {
        ...
        given(repository.obterSaldoPorTipoLancamentoEUsuarioEStatusEAnoEMes(
                eq(usuarioId), eq(TipoLancamento.RECEITA), eq(StatusLancamento.EFETIVADO)))
                .willReturn(new BigDecimal("150.00"));
        given(repository.obterSaldoPorTipoLancamentoEUsuarioEStatusEAnoEMes(
                eq(usuarioId), eq(TipoLancamento.DESPESA), eq(StatusLancamento.EFETIVADO)))
    ...
    void converterDTODevePopularLancamentoComCategoriasResolvidas() throws Exception {
        ...
        LancamentoDTO dto = LancamentoDTOFactory.create(null, "Desc", 9, 2025,
                new BigDecimal("123.45"), TipoLancamento.RECEITA.name(), StatusLancamento.PENDENTE.name());
        ...
        given(categoriaService.buscarPorNome(any(Categoria.class))).willAnswer(inv -> {
            ...
            if ("Mercado".equalsIgnoreCase(filtro.getNome())) return Collections.singletonList(categoriaMercado);
            if ("Transporte".equalsIgnoreCase(filtro.getNome())) return Collections.singletonList(categoriaTransporte);
    ...
    void validarStatusLancamentoDeveLancarQuandoNaoPendente() throws Exception {
        ...
        given(usuarioService.obterIdUsuarioPorEmail(email)).willReturn(usuario);
        given(repository.findLancamentoByIdAndUsuarioId(lancamentoId, usuario.getId())).willReturn(Optional.of(lancamentoEfetivado));
    ...
    void resolverCategoriasDoUsuarioDeveRetornarVazioParaEntradaNulaOuVazia() throws Exception {
        ...
        Set<Categoria> categoriasQuandoVazio = service.resolverCategoriasDoUsuario(Collections.emptyList(), authentication);
    ...
    void resolverCategoriasDoUsuarioDeveCriarQuandoNaoEncontrar() throws Exception {
        ...
        Set<Categoria> categoriasResolvidas = service.resolverCategoriasDoUsuario(Collections.singletonList("Nova"), authentication);
    ...
    void resolverCategoriasDoUsuarioDeveIgnorarNomesEmBranco() throws Exception {
        ...
        Set<Categoria> categoriasResolvidas = service.resolverCategoriasDoUsuario(Arrays.asList(" ", "\t", null, "Utilidade"), authentication);
