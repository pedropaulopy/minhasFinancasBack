...
class GoogleSheetsExportImplTest {
	...
	void createSheetFromCsv_deveCriarPlanilhaUsandoNomePadraoEComPastaPai() throws Exception {
		...
		assertThat(pixelSizeForColumn(dimensionRequests, 1)).isEqualTo(240);
		assertThat(pixelSizeForColumn(dimensionRequests, 2)).isEqualTo(150);
		assertThat(pixelSizeForColumn(dimensionRequests, 3)).isEqualTo(100);
		assertThat(pixelSizeForColumn(dimensionRequests, 4)).isEqualTo(275);
