...
class ImportResultadoDTOTest {
	...
	void contadoresEGettersFuncionam() {
		...
		List<AuxiliarLinhaErro> erros = dto.getErros();
		assertEquals(1, erros.size(), "erros size");
		AuxiliarLinhaErro e = erros.get(0);

		assertEquals(10L, e.getLinha());
		assertEquals("motivo X", e.getMotivo());
		assertEquals("raw line", e.getRaw());
		...
		assertEquals(11L, dto.getErros().get(1).getLinha());
	}
	...
	void listaDeErrosSoEhAlteradaViaAddFalha() {
		...
		assertThrows(UnsupportedOperationException.class, () -> {
			dto.getErros().add(new AuxiliarLinhaErro(0, "", ""));
		});
	}
