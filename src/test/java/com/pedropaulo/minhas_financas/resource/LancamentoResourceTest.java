...
class LancamentoResourceTest {
    ...
    @Mock private LancamentoCsvImportService importService;
    ...
    void salvar_deveRetornarBadRequest_quandoRegraNegocio() throws Exception {
        ...
        when(lancamentoService.converterDTO(any(LancamentoDTO.class), eq(authentication))).thenReturn(criarLancamentoNovo());
    ...
    void atualizar_deveRetornarOk_quandoSucesso() throws Exception {
        ...
        when(lancamentoService.atualizar(eq(lancamentoId), eq(authentication), eq(lancamentoDTO))).thenReturn(lancamentoAtualizado);
    ...
    void atualizarStatus_deveRetornarCreated_quandoSucesso() throws Exception {
        ...
        doNothing().when(lancamentoService).atualizarStatus(eq(lancamentoId), eq(authentication), eq(StatusLancamento.EFETIVADO));
    ...
    void buscar_deveRetornarOk_comListaVazia() throws Exception {
        ...
        ResponseEntity<List<Lancamento>> resposta = resource.buscar(
                "Aluguel", 10, 2025, BigDecimal.valueOf(1200),
